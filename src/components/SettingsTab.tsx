...
import { processedCSVDataAtom, productNameMappingsAtom, headerNamesAtom, currentCSVDataAtom, visibleColumnsAtom } from '../stores/csvStore';
...
export const SettingsTab: React.FC = () => {
  ...
  const allHeaders = currentCSVData.data.length > 0 ? Object.keys(currentCSVData.data[0]) : [];
    ...
    if (Object.keys(tempMappings).length > 0) {
      ...
      debounceTimerRef.current = setTimeout(async () => {
        try {
          ...
          // 저장된 매핑만큼 토스트 메시지 표시
          const changedCount = Object.keys(tempMappings).filter(key => tempMappings[key].trim() !== '').length;
          if (changedCount > 0) {
  ...
  }, [tempMappings, productNameMappings, currentCSVData, setProductNameMappings, setCurrentCSVData]);

  // 헤더명 변경을 위한 디바운스 로직
  useEffect(() => {
    // 초기값과 다른 경우에만 자동 저장 실행
    const hasChanges = Object.keys(tempHeaders).some(key => tempHeaders[key] !== headerNames[key]);
  ...
  return (
    <div className="p-4 space-y-6 bg-gray-100 dark:bg-gray-900 min-h-screen transition-colors">
      <h2 className="text-lg font-bold text-gray-800 dark:text-white transition-colors">설정</h2>

      {/* 헤더명 관리 섹션 */}
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow dark:shadow-gray-900/50 p-4 transition-colors">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-md font-semibold text-gray-800 dark:text-white transition-colors">헤더명 관리</h3>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1 transition-colors">
              CSV 파일의 컬럼명이 다를 경우 여기서 매핑을 설정하세요. 변경사항은 1초 후 자동으로 저장됩니다.
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 transition-colors">
              분류 컬럼 <span className="text-xs text-gray-500 dark:text-gray-400 transition-colors">(업체별 분류에 사용)</span>
            </label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 transition-colors">
              주소 컬럼 <span className="text-xs text-gray-500 dark:text-gray-400 transition-colors">(주문건을 하나로 묶는데 사용)</span>
            </label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 transition-colors">
              수량 컬럼 <span className="text-xs text-gray-500 dark:text-gray-400 transition-colors">(수량 계산에 사용)</span>
            </label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 transition-colors">
              가격 컬럼 <span className="text-xs text-gray-500 dark:text-gray-400 transition-colors">(가격 계산에 사용)</span>
            </label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 transition-colors">
              상품명 컬럼 <span className="text-xs text-gray-500 dark:text-gray-400 transition-colors">(상품명 변경에 사용)</span>
            </label>
            <input
              type="text"
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow dark:shadow-gray-900/50 p-4 transition-colors">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-md font-semibold text-gray-800 dark:text-white transition-colors">컬럼 표시 설정</h3>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1 transition-colors">
              발주서와 분류 탭에서 보여질 컬럼을 선택하세요. 변경사항은 즉시 저장되며 프린트에도 적용됩니다.
            </p>
          </div>
          <button
            onClick={() => {
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
          {allHeaders.map((header) => (
            ...
            >
              ...
              />
              <span className="text-sm text-gray-700 dark:text-gray-300 truncate transition-colors" title={header}>
                {header}
              </span>
            </label>
          ))}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow dark:shadow-gray-900/50 p-4 transition-colors">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-md font-semibold text-gray-800 dark:text-white transition-colors">상품명 관리</h3>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1 transition-colors">
              상품명을 다른 이름으로 변경할 수 있습니다. 변경사항은 1초 후 자동으로 저장되며 모든 탭에 실시간 반영됩니다.
            </p>
          </div>
          <button
        ...
        <div className="space-y-3 mb-4">
          {uniqueProductNames.map((productName) => {
            ...
            const currentMapping = productName in tempMappings 
            ...
            return (
              <div key={productName} className="flex items-center space-x-3">
                <div className="flex-1">
                  <div className="text-xs text-gray-500 dark:text-gray-400 mb-1 transition-colors">원본명</div>
                ...
                <div className="flex-1">
                  <div className="text-xs text-gray-500 dark:text-gray-400 mb-1 transition-colors">변경명</div>
                  <input
                    type="text"
